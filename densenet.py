...
'''
@Create Time : 2021/8/9 16:51 
@Author : Gabriel
@File : densenet.py 
@Project:ImageClassification
@About : DenseNet网络基础部件
'''
...
class BottleNeck(layers.Layer):
    ...
    def call(self, x):
        ...
        '''
        将所有输出feature map按Channel组合在一起.这里所用到的非线性变换H为BN+ReLU+ Conv(3×3)的组合
        例如，X 的 shape 为 [4, 8, 8, 3]，输出 Y 的 shape 为 [4, 8, 8, 32]，那么 [X, Y] 的 shape 为 [4, 8, 8, 35]。
        '''
...
class DenseNet(tf.keras.Model):
    ...
    # drop_rate：失活率
    def __init__(self, num_classes, num_init_features, growth_rate, block_layers, compression_rate, drop_rate):
        ...
        # 全连接层，进行10分类
        self.fc = layers.Dense(units=num_classes, activation=tf.keras.activations.softmax)
